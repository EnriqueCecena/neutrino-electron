...
def work_file(my_path,react_dict,sim_file):
    ...
    #In the case of enriched neutrino files, the event ids and vertex ids are generally the same.
    #But in this case we won't use the enriched files. 
    #We grab the info of the event from the mc header
        info_event=mc_hdr[mc_hdr['event_id']==id]
        event=segments[segments['event_id']==id]
        vertex_ids, counts=np.unique(event['vertex_id'],return_counts=True)
        for vid in vertex_ids:
            vert_event=event[event['vertex_id']==vid]
            filt_vert=vert_event[vert_event['dE']>1]
            info_vertex=info_event[info_event['vertex_id']==vid]

        #asign the reaction and the charge
            reaction=info_vertex['reaction']
            boolean=info_vertex['isCC']
            
        #Now that we've filtered the info we must generate the images.
            grid, xedges,yedges= np.histogram2d(filt_vert['z'],filt_vert['y'],bins=[x_bins,y_bins],weights=filt_vert['dE'])
...
#Now we need to import the file and send it to process
    sim_file='/home/enrique/Documents/neutrino-electron/data/MicroProdN3p1_NDLAr_2E18_FHC.convert2h5.nu.0000006.EDEPSIM.hdf5'
