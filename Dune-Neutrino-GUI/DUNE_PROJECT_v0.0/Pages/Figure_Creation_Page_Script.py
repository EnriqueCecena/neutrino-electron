...
class Figure_Creation_Page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.file_combobox = ttk.Combobox(
            ...
            values=controller.Allowed_Files,
            state='readonly',
            width=60
        )
        ...
        self.cmap_yes_no.trace_add(
            'write',
            lambda *args: self.Lock_Unlock_Cmap(self.cmap_yes_no, self.cmap_selection_combobox)
        )
        ...
        if self.controller.plot_type == 'scatter':
            ...
            self.dropdown_3d = ttk.Combobox(
                self.Dropdown_3D_frame,
                textvariable=self.dropdown_3d_select,
                values=['No', 'Yes'],
                width=10
            )
            self.dropdown_3d.pack(side=tk.LEFT)
            self.dropdown_3d_select.trace_add(
                'write',
                lambda *args: self.Lock_Unlock_Cmap(self.dropdown_3d_select, self.z_combobox)
            )
        ...
        if self.controller.plot_type == 'hist':
            ...
            self.group_yes_no.trace_add(
                'write',
                lambda *args: self.Lock_Unlock_Cmap(self.group_yes_no, self.hist_selection_combobox)
            )
        ...
        self.Create_Fig_Button = tk.Button(
            self,
            text='Create',
            command=lambda: self.Plot_Type_Map()
        )
